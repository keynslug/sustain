...
--
-- @
--'delete' \"http:\/\/httpbin.org\/delete\" mempty
-- @
--
-- >>> r <- delete "http://httpbin.org/delete" mempty
-- >>> r ^. responseStatus . statusCode
-- 200
delete :: Deletable a => String -> a -> IO (Response L.ByteString)
...
--'deleteWith' opts \"http:\/\/httpbin.org\/delete\" mempty
-- @
--
-- >>> let opts = defaults & redirects .~ 0
-- >>> r <- deleteWith opts "http://httpbin.org/delete" mempty
-- >>> r ^. responseStatus . statusCode
-- 200
deleteWith :: Deletable a => Options -> String -> a -> IO (Response L.ByteString)
deleteWith opts url payload = runRead =<< prepareDelete opts url payload
