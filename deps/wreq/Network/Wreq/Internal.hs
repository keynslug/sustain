...
import Network.Wreq.Types (Auth(..), Options(..), Postable(..), Putable(..), Deletable(..))
...
prepareDelete :: Deletable a => Options -> String -> a -> IO Req
prepareDelete opts url payload = Req (manager opts) <$>
