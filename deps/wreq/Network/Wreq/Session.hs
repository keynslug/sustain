...
import Network.Wreq.Types (Postable, Putable, Deletable, Run)
...
delete :: Deletable a => Session -> String -> a -> IO (Response L.ByteString)
...
deleteWith :: Deletable a => Options -> Session -> String -> a -> IO (Response L.ByteString)
deleteWith opts sesh url payload = run string sesh =<< prepareDelete opts url payload
